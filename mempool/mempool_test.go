...
func (s *fakeChain) FetchUtxoView(tx *czzutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *czzutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *czzutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   czzutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
type poolHarness struct {
	...
	signKey     *czzec.PrivateKey
	payAddr     czzutil.Address
	payScript   []byte
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*czzutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32) (*czzutil.Tx, error) {
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*czzutil.Tx, error) {
	txChain := make([]*czzutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, czzutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	signKey, signPub := czzec.PrivKeyFromBytes(czzec.S256(), keyBytes)

	// Generate associated pay-to-script-hash address and resulting payment
	// script.
	pubKeyBytes := signPub.SerializeCompressed()
	payPubKeyAddr, err := czzutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
func testPoolMembership(tc *testContext, tx *czzutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   czzutil.Amount(5000000000),
