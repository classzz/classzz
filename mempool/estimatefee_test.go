...
func (eft *estimateFeeTester) testTx(fee czzutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: czzutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func expectedFeePerKilobyte(t *TxDesc) CzzPerKilobyte {
	size := float64(t.TxDesc.Tx.MsgTx().SerializeSize())
	fee := float64(t.TxDesc.Fee)

	return SatoshiPerByte(fee / size).ToCzzPerKb()
}
...
func (eft *estimateFeeTester) estimates() [estimateFeeDepth]CzzPerKilobyte {

	// Generate estimates
	var estimates [estimateFeeDepth]CzzPerKilobyte
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	estimateHistory [][estimateFeeDepth]CzzPerKilobyte,
	txPerRound, txPerBlock uint32) ([][]*TxDesc, [][estimateFeeDepth]CzzPerKilobyte) {

	// generate new txs.
	var newTxs []*TxDesc
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(czzutil.Amount(rand.Intn(1000000)))
...
func TestEstimateFeeRollback(t *testing.T) {
	...
	estimateHistory := [][estimateFeeDepth]CzzPerKilobyte{eft.estimates()}
...
func (eft *estimateFeeTester) checkSaveAndRestore(
	previousEstimates [estimateFeeDepth]CzzPerKilobyte) {
