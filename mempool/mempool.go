...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in CZZ/kB to be
...
func (mp *TxPool) removeOrphan(tx *czzutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *czzutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *czzutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*czzutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *czzutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *czzutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *czzutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *czzutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *czzutil.Tx) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
func (mp *TxPool) CheckSpend(op wire.OutPoint) *czzutil.Tx {
	mp.mtx.RLock()
	txR := mp.outpoints[op]
	mp.mtx.RUnlock()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchInputUtxos(tx *czzutil.Tx) (*blockchain.UtxoViewpoint, error) {
	mp.mtx.RLock()
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*czzutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *czzutil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	txHash := tx.Hash()
...
func (mp *TxPool) validateBeaconTransaction(tx *czzutil.Tx, nextBlockHeight int32) error {
	...
	// AddBeaconPledge
	abp, err1 := cross.IsAddBeaconPledgeTx(tx.MsgTx(), mp.cfg.ChainParams)
	...
	if bpt != nil && mp.cfg.ChainParams.ExChangeHeight > nextBlockHeight {
		...
		if _, _, err := mp.cfg.ExChangeVerify.VerifyBurnProof(tx, bpt, eState, uint64(nextBlockHeight)); err != nil {
			return err
		}
	}
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *czzutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *czzutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*czzutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *czzutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
func (mp *TxPool) RawMempoolVerbose() map[string]*btcjson.GetRawMempoolVerboseResult {
	...
	for _, desc := range mp.pool {
		...
		mpd := &btcjson.GetRawMempoolVerboseResult{
			...
			Fee:              czzutil.Amount(desc.Fee).ToCZZ(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		outpoints:      make(map[wire.OutPoint]*czzutil.Tx),
	}
}
