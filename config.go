...
const (
	defaultConfigFilename          = "classzz.conf"
	defaultDataDirname             = "data"
	defaultLogLevel                = "info"
	defaultLogDirname              = "logs"
	defaultLogFilename             = "classzz.log"
	defaultMaxPeers                = 125
...
var (
	defaultHomeDir     = czzutil.AppDataDir("classzz", false)
...
// config defines the configuration options for classzz.
//
...
type config struct {
	...
	MinRelayTxFee           float64       `long:"minrelaytxfee" description:"The minimum transaction fee in CZZ/kB to be considered a non-zero fee."`
	...
	RejectNonStd            bool          `long:"rejectnonstd" description:"RejFect non-standard transactions regardless of the default settings for the active network."`
...
//
// The above results in classzz functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		MinRelayTxFee:           mempool.DefaultMinRelayTxFee.ToCZZ(),
	...
	cfg.minRelayTxFee, err = czzutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	if configFileError != nil {
		czzdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the sample-classzz.conf content to the given destination path,
...
func createDefaultConfigFile(destinationPath string) error {
	...
	sampleBytes, err := Asset("sample-classzz.conf")
	if err != nil {
		return err
	}
...
// czzdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func czzdDial(addr net.Addr) (net.Conn, error) {
...
// czzdLookup resolves the IP of the given host using the correct DNS lookup
...
func czzdLookup(host string) ([]net.IP, error) {
