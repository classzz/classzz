...
func (mock *MockPeerNotifier) TransactionConfirmed(tx *czzutil.Tx) {
...
// convenience.
func GenerateAnyoneCanSpendAddress(chainParams *chaincfg.Params) (czzutil.Address, []byte, error) {
	redeemScript := []byte{txscript.OP_TRUE}
	scriptSig, err := txscript.NewScriptBuilder().AddData(redeemScript).Script()
	if err != nil {
		return nil, nil, err
	}
	address, err := czzutil.NewAddressScriptHash(redeemScript, chainParams)
	if err != nil {
		return nil, nil, err
	}
