...
const (
	// svcName is the name of classzz service.
	svcName = "classzzsvc"
	...
	svcDisplayName = "Classzz Service"
...
// czzdService houses the main service handler which handles all service
// updates and launching czzdMain.
type czzdService struct{}
...
// long-running czzdMain (which is the real meat of classzz), handles service
// change requests, and notifies the service control manager of changes.
func (s *czzdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start czzdMain in a separate goroutine so the service can start
...
// installService attempts to install the classzz service.  Typically this should
...
// removeService attempts to uninstall the classzz service.  Typically this should
...
// startService attempts to start the classzz service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &czzdService{})
	if err != nil {
