...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *czzutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *czzutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *czzutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a czzutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := czzutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a czzutil.Address) bool {
	switch a := a.(type) {
	case *czzutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a czzutil.Address) {
	switch a := a.(type) {
	case *czzutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected czzutil.Block
type notificationBlockDisconnected czzutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *czzutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *czzutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *czzutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, czzutil.Amount(amount).ToCZZ())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*czzutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *czzutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *czzutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *czzutil.Tx, block *czzutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *czzutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *czzutil.Tx, block *czzutil.Block) {
...
out:
	for {
		...
		//
		// Classzz does not respond to any request without and "id" or "id":null,
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := czzutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *czzutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *czzutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *czzutil.Block) error {
...
fetchRange:
	...
	// Notify websocket client of the finished rescan.  Due to how classzz
