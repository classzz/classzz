...
type Config struct {
	...
	ProcessBlock func(*czzutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *czzutil.Block) bool {
...
func (m *CPUMiner) solveBlock2(msgBlock *wire.MsgBlock, blockHeight int32,
...
func (m *CPUMiner) solveBlock(msgBlock *wire.MsgBlock, blockHeight int32,
	...
	lastTxUpdate := m.g.TxSource().LastUpdated()
	hashesCompleted := uint64(0)
	log.Info("miner block", "hash", param.Info.HeadHash.String(), "number", blockHeight, "target", hex.EncodeToString(param.Info.Target.Bytes()), "workSum", blockchain.CalcWork(header.Bits))
...
out:
	for {
		...
		select {
		case <-quit:
			//fmt.Println("generateBlocks quit")
			break out
		default:
			// Non-blocking select to fall through
		}
...
func (m *CPUMiner) miningWorkerController() {
	...
	launchWorkers := func(numWorkers uint32) {
		for i := uint32(0); i < (numWorkers - 1); i++ {
