...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr czzutil.Address) (*czzutil.Tx, error) {
	...
	if nextBlockHeight >= params.EntangleHeight && int64(nextBlockHeight) < cross.StartMergeBeaconUtxoHeight{
...
mempoolLoop:
	...
	if g.chainParams.EntangleHeight <= nextBlockHeight {
		err = cross.MakeMergeCoinbaseTx(coinbaseTx.MsgTx(), poolItem, entangleItems, rewards, mergeItems)
		if err != nil {
			return nil, nil, err
		}
	}
