...
// necessary to serve the RPCs and implements the czzrpc.proto interface.
...
func NewGrpcServer(cfg *GrpcServerConfig) *GrpcServer {
	...
	pb.RegisterCzzrpcServer(cfg.Server, s)
	serviceMap["czzrpc"] = s
	return s
}
...
func (s *GrpcServer) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		block, ok := notification.Data.(*czzutil.Block)
		if !ok {
...
func (s *GrpcServer) GetBlockchainInfo(ctx context.Context, req *pb.GetBlockchainInfoRequest) (*pb.GetBlockchainInfoResponse, error) {
	...
	case &chaincfg.TestNetParams:
		net = pb.GetBlockchainInfoResponse_TESTNET
...
func (s *GrpcServer) GetRawBlock(ctx context.Context, req *pb.GetRawBlockRequest) (*pb.GetRawBlockResponse, error) {
	...
	if err := block.MsgBlock().CzzEncode(&buf, wire.ProtocolVersion, wire.BaseEncoding); err != nil {
...
func (s *GrpcServer) GetTransaction(ctx context.Context, req *pb.GetTransactionRequest) (*pb.GetTransactionResponse, error) {
	...
	respTx := marshalTransaction(czzutil.NewTx(&msgTx), s.chain.BestSnapshot().Height-blockHeight, &header, blockHeight, s.chainParams)
	if s.txIndex != nil {
...
func (s *GrpcServer) GetRawTransaction(ctx context.Context, req *pb.GetRawTransactionRequest) (*pb.GetRawTransactionResponse, error) {
	...
	if tx, err := s.txMemPool.FetchTransaction(txHash); err == nil {
		var buf bytes.Buffer
		if err := tx.MsgTx().CzzEncode(&buf, wire.ProtocolVersion, wire.BaseEncoding); err != nil {
...
func (s *GrpcServer) GetAddressTransactions(ctx context.Context, req *pb.GetAddressTransactionsRequest) (*pb.GetAddressTransactionsResponse, error) {
	...
	addr, err := czzutil.DecodeAddress(req.Address, s.chainParams)
	if err != nil {
	...
	for _, cTx := range confirmedTxs {
		tx := marshalTransaction(czzutil.NewTx(&cTx.tx), tip-cTx.blockHeight+1, cTx.blockHeader, cTx.blockHeight, s.chainParams)
		if s.txIndex != nil {
...
func (s *GrpcServer) GetRawAddressTransactions(ctx context.Context, req *pb.GetRawAddressTransactionsRequest) (*pb.GetRawAddressTransactionsResponse, error) {
	...
	for _, uTx := range unconfirmedTxs {
		var buf bytes.Buffer
		if err := uTx.MsgTx().CzzEncode(&buf, wire.ProtocolVersion, wire.BaseEncoding); err != nil {
...
func (s *GrpcServer) GetAddressUnspentOutputs(ctx context.Context, req *pb.GetAddressUnspentOutputsRequest) (*pb.GetAddressUnspentOutputsResponse, error) {
	...
	for {
		...
		for _, cTx := range confirmedTxs {
			...
			utxoView, err := s.chain.FetchUtxoView(czzutil.NewTx(&cTx.tx))
			if err != nil {
				return nil, err
			}
...
func (s *GrpcServer) GetMerkleProof(ctx context.Context, req *pb.GetMerkleProofRequest) (*pb.GetMerkleProofResponse, error) {
	...
	// encode proof to hex
	var buf bytes.Buffer
	if err := mBlock.CzzEncode(&buf, wire.ProtocolVersion, wire.LatestEncoding); err != nil {
...
func (s *GrpcServer) setInputMetadata(tx *pb.Transaction) error {
	...
	for i, in := range tx.Inputs {
		...
		if prevTx, ok := inputTxMap[*ch]; ok {
			...
			if err := loadedTx.CzzDecode(bytes.NewReader(txBytes), wire.ProtocolVersion, wire.BaseEncoding); err != nil {
...
func (s *GrpcServer) fetchTransactionsByAddress(addr czzutil.Address, startHeight int32, nbFetch, nbSkip int) ([]retrievedTx, []*czzutil.Tx, error) {
...
func marshalBlockInfo(block *czzutil.Block, confirmations int32, params *chaincfg.Params) *pb.BlockInfo {
	return &pb.BlockInfo{
...
func marshalTransaction(tx *czzutil.Tx, confirmations int32, blockHeader *wire.BlockHeader, blockHeight int32, params *chaincfg.Params) *pb.Transaction {
