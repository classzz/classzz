...
func (f *txFilter) AddAddress(addr czzutil.Address) {
	switch a := addr.(type) {
...
func (f *txFilter) RemoveAddress(addr czzutil.Address) {
	switch a := addr.(type) {
	case *czzutil.AddressPubKeyHash:
...
func (f *txFilter) AddRPCFilter(rpcFilter *pb.TransactionFilter, params *chaincfg.Params) error {
	...
	for _, addrStr := range rpcFilter.GetAddresses() {
		addr, err := czzutil.DecodeAddress(addrStr, params)
		if err != nil {
...
func (f *txFilter) MatchAndUpdate(tx *czzutil.Tx, params *chaincfg.Params) bool {
