...
compile:
	...
	gox -osarch="$(OSARCH)" -output "$(OUTDIR)/$(APPNAME)-{{.OS}}_{{.Arch}}/$(CLINAME)" ./cmd/czzctl
	@for dir in $(DIRS) ; do \
		(cp README.md $(OUTDIR)/$(APPNAME)-$$dir/README.md) ;\
		(cp LICENSE $(OUTDIR)/$(APPNAME)-$$dir/LICENSE) ;\
		(cp sample-classzz.conf $(OUTDIR)/$(APPNAME)-$$dir/sample-classzz.conf) ;\
...
install:
	go install .
	go install ./cmd/czzctl

uninstall:
	go clean -i
	go clean -i ./cmd/czzctl
