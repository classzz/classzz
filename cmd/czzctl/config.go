...
var (
	...
	czzctlHomeDir         = czzutil.AppDataDir("czzctl", false)
	czzwalletHomeDir      = czzutil.AppDataDir("czzwallet", false)
	defaultConfigFile     = filepath.Join(czzctlHomeDir, "czzctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(czzdHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(czzwalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for czzctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(czzctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		// Use config file for RPC server to create default czzctl config
		var serverConfigPath string
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(czzwalletHomeDir, "czzwallet.conf")
		} else {
			serverConfigPath = filepath.Join(czzdHomeDir, "classzz.conf")
		}
...
// For this it tries to read the config file for the RPC server (either classzz or
// czzwallet), and extract the RPC user and password from it.
