...
type memWallet struct {
	coinbaseKey  *czzec.PrivateKey
	coinbaseAddr czzutil.Address
	...
	addrs map[uint32]czzutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]czzutil.Address)
...
// SetRPCClient saves the passed rpc connection to classzz as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*czzutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          czzutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (czzutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []czzutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt czzutil.Amount,
	feeRate czzutil.Amount, change bool) ([]czzutil.Amount, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt czzutil.Amount
	for _, output := range outputs {
		outputAmt += czzutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
	...
	for i, txIn := range tx.TxIn {
		...
		sigScript, err := txscript.SignatureScript(tx, i,
			int64(inputAmounts[i].ToUnit(czzutil.AmountSatoshi)), utxo.pkScript,
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() czzutil.Amount {
	m.RLock()
	defer m.RUnlock()
...
func keyToAddr(key *czzec.PrivateKey, net *chaincfg.Params) (czzutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := czzutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
