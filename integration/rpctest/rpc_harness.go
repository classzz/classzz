...
// Harness fully encapsulates an active classzz process to provide a unified
// platform for creating rpc driven integration tests involving classzz. The
// active classzz node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active classzz process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*czzutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the classzz node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []czzutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure classzz properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created classzz
...
func (h *Harness) NewAddress() (czzutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() czzutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate czzutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate czzutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*czzutil.Tx, blockVersion int32,
	blockTime time.Time) (*czzutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*czzutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*czzutil.Block, error) {
	...
	mBlock, err := h.Node.GetBlock(prevBlockHash.String())
	if err != nil {
		return nil, err
	}
	prevBlock := czzutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "classzz", "rpctest")
