...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt czzutil.Amount) *chainhash.Hash {
		...
		// Next, send amt CZZ to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(czzutil.Amount(5 * czzutil.SatoshiPerBitcoin))
	...
	txid = genSpend(czzutil.Amount(500 * czzutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(czzutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*czzutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, czzutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 CZZ.
	expectedBalance := czzutil.Amount(250 * czzutil.SatoshiPerBitcoin)
	...
	// The original wallet should now have a balance of 0 CZZ as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = czzutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := czzutil.Amount(50 * czzutil.SatoshiPerBitcoin)
	...
	// The current wallet balance should now be at least 50 CZZ less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 CZZ) of mature unspendable
	// outputs.
	expectedBalance := czzutil.Amount(numMatureOutputs * 50 * czzutil.SatoshiPerBitcoin)
