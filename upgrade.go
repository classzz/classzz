...
// oldBchdHomeDir returns the OS specific home directory classzz used prior to
// version 0.3.3.  This has since been replaced with czzutil.AppDataDir, but
...
// location prior to classzz version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to classzz
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "classzz.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "classzz_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "classzz_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to classzz
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		czzdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old classzz.conf into new location if needed.
		...
		if ohpEmpty {
			...
			czzdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to classzz as new versions require it.
