...
5. [How do I use bootstrap.dat with classzz?](#Importing)
...
**NOTE:** Using bootstrap.dat is entirely optional.  Classzz will download the
...
### 5. How do I use bootstrap.dat with classzz?

classzz comes with a separate utility named `addblock` which can be used to import
...
1. Stop classzz if it is already running.  This is required since addblock needs to
   access the database used by classzz and it will be locked if classzz is using it.
...
**Windows:**
```bat
C:\> "%PROGRAMFILES%\bourbaki-czz\classzz\addblock" -i C:\Path\To\bootstrap.dat
```
