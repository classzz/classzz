...
classzz provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, classzz supports a hybrid,
...
Configuring classzz as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the classzz command
line or in the classzz configuration file.  Typically the Tor proxy address will be
...
By default, classzz assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since classzz will act as the server.
...
```text
HiddenServiceDir /var/tor/classzz
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring classzz as a Tor hidden service requires
three flags:
...
classzz provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the classzz command line or in the classzz
...
classzz provides support for Tor stream isolation by using the `--torisolation`
