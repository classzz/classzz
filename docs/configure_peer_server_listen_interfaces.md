classzz allows you to bind to specific interfaces which enables you to setup
...
The following config file would configure classzz to only listen on localhost for both IPv4 and IPv6:
...
In addition, if you are starting czzd with TLS and want to make it
available via a hostname, then you will need to generate the TLS
certificates for that host. For example,
 ```
gencerts --host=myhostname.example.com --directory=/home/user/.classzz/
```
