...
classzz provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between classzz and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, classzz intentionally splits the wallet and chain
  ...
  talking directly to classzz, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [czzwallet](https://github.com/bourbaki-czz/czzwallet).
* classzz is secure by default which means that the RPC connection is TLS-enabled
  by default
* classzz provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for classzz RPC and are used by applications
such as [czzwallet](https://github.com/bourbaki-czz/czzwallet) for inter-process
communication with classzz.  The websocket connection endpoint for classzz is
...
during the development of czzwallet.
...
The classzz RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
classzz is `wss://your_ip_or_domain:8334/ws`.
...
to a classzz RPC server:

* **rpcuser** is the full-access username configured for the classzz RPC server
* **rpcpass** is the full-access password configured for the classzz RPC server
* **rpclimituser** is the limited username configured for the classzz RPC server
* **rpclimitpass** is the limited password configured for the classzz RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the classzz
  server is configured with.  It is automatically generated by classzz and placed
  in the classzz home directory (which is typically `%LOCALAPPDATA%\classzz` on
  Windows and `~/.classzz` on POSIX-like OSes)

**NOTE:** As mentioned above, classzz is secure by default which means the RPC
...
The classzz RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
classzz comes with a separate utility named `czzctl` which can be used to issue
these RPC commands via HTTP POST requests to classzz after configuring it with the
...
|25|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">classzz does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|26|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since classzz does not have the wallet integrated to provide payment addresses, classzz must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|27|[stop](#stop)|N|Shutdown classzz.|
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a classzz extension**</font>|
...
|Notes|<font color="orange">Since classzz does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since classzz does not have the wallet integrated to provide payment addresses, classzz must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">classzz does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown classzz.|
|Returns|`"classzz stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For classzz this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">classzz currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for classzz.</font>|
...
The following is an overview of the RPC methods which are implemented by classzz, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network classzz is running on.|None|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `classzz`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|string|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR classzz CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get bitcoin network classzz is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of classzz.|
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this classzz instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
classzz uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll classzz for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/bourbaki-czz/classzz/rpcclient) package.
...
[rpcclient](https://github.com/bourbaki-czz/classzz/rpcclient) package to connect with
a classzz instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by classzz when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/bourbaki-czz/classzz/rpcclient) package to connect with
a classzz instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/bourbaki-czz/classzz/rpcclient) package to connect with
a classzz instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a classzz instance,
...
// generated by classzz when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/classzz/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The classzz generated certificate acts as
