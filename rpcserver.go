...
// should ask a connected instance of czzwallet.
...
// These commands will be implemented in czzwallet.
...
func messageToHex(msg wire.Message) (string, error) {
	...
	if err := msg.CzzEncode(&buf, maxProtocolVersion, wire.BaseEncoding); err != nil {
...
func handleBeaconRegistration(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if c.BeaconRegistration.StakingAmount < float64(s.cfg.ChainParams.MinStakingAmount.Uint64()) || c.BeaconRegistration.StakingAmount > czzutil.MaxSatoshi {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
...
func handleAddBeaconPledge(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	// Convert the amount to satoshi.
	satoshi, err := czzutil.NewAmount(c.AddBeaconPledge.StakingAmount)
	if err != nil {
		context := "Failed to convert amount"
		return nil, internalRPCError(err.Error(), context)
	}
	...
	if c.AddBeaconPledge.StakingAmount < float64(s.cfg.ChainParams.MinAddStakingAmount.Uint64()) || c.AddBeaconPledge.StakingAmount > czzutil.MaxSatoshi {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
			Message: "Invalid StakingAmount",
		}
	...
	params := s.cfg.ChainParams
	addr, err := czzutil.NewLegacyAddressPubKeyHash(c.AddBeaconPledge.ToAddress, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
func handleMortgage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	piByte, err := rlp.EncodeToBytes(pi)
	scriptInfo, err := txscript.MortgageScript(piByte)
	if err != nil {
		return nil, err
	}
	...
	if c.Mortgage.StakingAmount < float64(s.cfg.ChainParams.MinStakingAmount.Uint64()) || c.Mortgage.StakingAmount > czzutil.MaxSatoshi {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
			Message: "Invalid StakingAmount",
		}
	...
	params := s.cfg.ChainParams
	addr, err := czzutil.NewLegacyAddressPubKeyHash(c.Mortgage.ToAddress, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// Convert the amount to satoshi.
	satoshi, err := czzutil.NewAmount(c.Mortgage.StakingAmount)
	if err != nil {
		context := "Failed to convert amount"
		return nil, internalRPCError(err.Error(), context)
	}
...
func handleAddMortgage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*btcjson.AddMortgageCmd)
	...
	// Convert the amount to satoshi.
	satoshi, err := czzutil.NewAmount(c.AddMortgage.StakingAmount)
	if err != nil {
		context := "Failed to convert amount"
		return nil, internalRPCError(err.Error(), context)
	}
	...
	if c.AddMortgage.StakingAmount < float64(s.cfg.ChainParams.MinAddStakingAmount.Uint64()) || c.AddMortgage.StakingAmount > czzutil.MaxSatoshi {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
			Message: "Invalid StakingAmount",
		}
	...
	params := s.cfg.ChainParams
	addr, err := czzutil.NewLegacyAddressPubKeyHash(c.AddMortgage.ToAddress, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
func handleUpdateCoinbaseAll(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*btcjson.UpdateCoinbaseAllCmd)
	...
	abc := &cross.UpdateCoinbaseAll{
		CoinBaseAddress: c.UpdateCoinbase.CoinBaseAddress,
	}

	BeaconByte, err := rlp.EncodeToBytes(abc)
	scriptInfo, err := txscript.UpdateCoinbaseAllScript(BeaconByte)
	if err != nil {
		return nil, err
	}
...
func handleConvert(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*btcjson.ConvertCmd)
...
func handleCasting(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
...
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []btcjson.Vout {
	...
	for i, v := range mtx.TxOut {
		...
		vout.N = uint32(i)
		vout.Value = czzutil.Amount(v.Value).ToCZZ()
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := czzutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := czzutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	if *c.Verbosity == 1 {
		...
		for i, tx := range txns {
			rawTxn, err := createTxRawResult(params, tx.MsgTx(),
				tx.Hash().String(), blockHeader, c.Hash,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	txOutReply := &btcjson.GetTxOutResult{
		...
		Value:         czzutil.Amount(value).ToCZZ(),
...
func handleGetTxOutProof(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	block, err := czzutil.NewBlockFromBytes(blkBytes)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCDeserialization,
	...
	// encode proof to hex
	var buf bytes.Buffer
	err = mBlock.CzzEncode(&buf, wire.ProtocolVersion, wire.LatestEncoding)
...
func handleVerifyTxOutProof(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	err = msg.CzzDecode(rbuf, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCDeserialization,
			Message: "CzzDecode error: " + err.Error(),
		}
	}
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &btcjson.PrevOut{
				...
				Value:     czzutil.Amount(originTxOut.Value).ToCZZ(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr czzutil.Address, numToSkip, numRequested uint32) ([]*czzutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	params := s.cfg.ChainParams
	addr, err := czzutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
...
func handleSubmitWork(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if template == nil || template.Block.Header.BlockHashNoNonce().String() != c.Hash {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCVerify,
			Message: "The hashes don't match",
		}
	}
	...
	template.Block.Header.Nonce = c.Nonce
	block := czzutil.NewBlock(template.Block)
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	addr, err := czzutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func verifyChain(s *rpcServer, level, depth int32) error {
	...
	for height := best.Height; height > finishHeight; height-- {
		...
		if magneticAnomalyActive {
			prevHeader, _ = s.cfg.Chain.HeaderByHash(&block.MsgBlock().Header.PrevBlock)
		}
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if _, ok := addr.(*czzutil.AddressPubKeyHash); !ok {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCType,
	...
	address, err := czzutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="classzz RPC"`)
...
func genCertPair(certFile, keyFile string, externalIPs []string) error {
	...
	org := "classzz autogenerated cert"
	validUntil := time.Now().Add(10 * 365 * 24 * time.Hour)
	cert, key, err := czzutil.NewTLSCertPair(org, validUntil, externalIPs)
	if err != nil {
		return err
	}
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *czzutil.Block, flags blockchain.BehaviorFlags) (bool, error)
