...
func (sp *serverPeer) OnVersion(_ *peer.Peer, msg *wire.MsgVersion) *wire.MsgReject {
	...
	if strings.Contains(msg.UserAgent, "classzz:2.0") || strings.Contains(msg.UserAgent, "classzz:1.0") || strings.Contains(msg.UserAgent, "classzz:0.0") {
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a czzutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := czzutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a czzutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := czzutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) TransactionConfirmed(tx *czzutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			czzdLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new classzz server configured to listen on addr for the
...
func newServer(listenAddrs []string, db database.DB, chainParams *chaincfg.Params, interrupt <-chan struct{}) (*server, error) {
	...
	amgr := addrmgr.New(cfg.DataDir, czzdLookup)
	...
	if !cfg.DisableRPC {
		...
		s.gRPCServer, err = newGrpcServer(gRPCNetAddrs, &czzrpc.GrpcServerConfig{
			TimeSource:  s.timeSource,
			Chain:       s.chain,
			ChainParams: chainParams,
			DB:          db,
